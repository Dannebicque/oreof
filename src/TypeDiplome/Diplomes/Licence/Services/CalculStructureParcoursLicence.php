...
final class CalculStructureParcoursLicence
{
    ...
    public function calcul(Parcours $parcours, OptionsCalculStructure $optionsCalculStructure): StructureParcours
    {
        if ($this->parcoursRepository !== null) {
            $parcours = $this->parcoursRepository->find($parcours->getId());
        }

        $dtoStructure = StructureParcours::fromEntity($parcours, $optionsCalculStructure->withEcts, $optionsCalculStructure->withBcc);
//        $dtoStructure->setParcours($parcours);

        foreach ($parcours->getSemestreParcours() as $semestreParcours) {
            $dtoSemestre = $this->calculSemestre($semestreParcours, $optionsCalculStructure, $parcours, $dtoStructure);
            if ($dtoSemestre !== null) {
    ...
    public function calculSemestre(mixed $semestreParcours, OptionsCalculStructure $optionsCalculStructure, Parcours $parcours, ?StructureParcours $dtoStructure = null): ?StructureSemestre
    {
        if ($dtoStructure === null) {
            //todo: à voir pour ne pas avoir le dto des fiches dans la structure c'est ca le soucis...
            $dtoStructure = StructureParcours::fromEntity($parcours);
        }
            ...
            foreach ($ues as $ue) {
                if ($ue !== null && $ue->getUeParent() === null) {
                    ...
                    foreach ($ecs as $elementConstitutif) {
                        if ($elementConstitutif !== null && $elementConstitutif->getEcParent() === null) {
                            //récupérer le bon EC selon tous les liens
                            $dtoEc = new StructureEc($elementConstitutif, $parcours, false, $optionsCalculStructure->withEcts, $optionsCalculStructure->withBcc, $optionsCalculStructure->dataFromFicheMatiere);
                            $dtoStructure->statsFichesMatieresParcours->addEc($elementConstitutif, $raccrocheUe);
                            foreach ($elementConstitutif->getEcEnfants() as $elementConstitutifEnfant) {
                                $dtoEcEnfant = new StructureEc($elementConstitutifEnfant, $parcours, false, $optionsCalculStructure->withEcts, $optionsCalculStructure->withBcc, $optionsCalculStructure->dataFromFicheMatiere);
                    ...
                    foreach ($ue->getUeEnfants() as $ueEnfant) {
                        ...
                        if ($ueEnfant !== null) {
                            $dtoUeEnfant = new StructureUe($ueEnfant, $raccrocheUeEnfant, $display, $ueOrigineEnfant ?? null, $optionsCalculStructure->withEcts, $optionsCalculStructure->withBcc);
                            ...
                            foreach ($ueEnfant->getUeEnfants() as $ueEnfant2) {
                                ...
                                if ($ueEnfant2 !== null) {
                                    $dtoUeEnfant2 = new StructureUe($ueEnfant2, $raccrocheUeEnfant2, $display2, $ueOrigineEnfant2 ?? null, $optionsCalculStructure->withEcts, $optionsCalculStructure->withBcc);
